...
from web_app.src.schemas import UserCreate, VerifyRequest, EmailBase, PasswordResetConfirmRequest
from web_app.src.crud import (create_user, get_user_by_username, get_user_by_email,
                              update_password_user_by_email)
...
async def register(
    ...
    try:
        if await get_user_by_username(username=user.username) is not None:
            return {
                "unique": False,
                "message": "Username already exists"
            }

        if await get_user_by_email(email=user.email) is not None:
            return {
                "unique": False,
...
async def confirm_password_reset(request: PasswordResetConfirmRequest):
    ...
    if user.get("password_hash") == new_password_hash:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    # Обновляем пароль
    await update_password_user_by_email(
        email=user_email,
